...
class RBF_Net:
    ...
    def avg_cluster_dist(self, data, centroids, cluster_assignments, kmeans_obj):
        ...
        for i in range(self.k):
            newdata = data[cluster_assignments == i][:]
            dist = kmeans_obj.dist_pt_to_centroids(newdata, centroids[i])
    ...
    def linear_regression(self, A, y):
        ...
        '''
        matrix = np.hstack((A, np.ones((len(A), 1))))
        c, sse, rnk, s = scipy.linalg.lstsq(matrix, y)
        return c
    ...
    def hidden_act(self, data):
        ...
        for i in range(data.shape[0]):
            dist = kmeans_obj.dist_pt_to_centroids(self.prototypes, data[i])**2
            act = -dist / (2*self.sigmas**2) + small
            hiddenact.append(np.exp(act))
        return np.array(hiddenact)
    ...
    def output_act(self, hidden_acts):
        ...
        acts = np.hstack((hidden_acts, bias))
        outputact = acts @ self.wts
        return outputact
    ...
    def train(self, data, y):
        ...
        A = self.hidden_act(data)
        for i in range(self.num_classes):
            c = self.linear_regression(A, y)
            w.append(c)
        self.wts = np.array(w).reshape((self.k+1,self.num_classes))
    ...
    def predict(self, data):
        ...
        pred_y = []
        self.initialize(data)
        hiddenacts = self.hidden_act(data)
        outputact = self.output_act(hiddenacts)
        for i in range(data.shape[0]):
            if np.argmax(outputact[i]) == 0:
                pred_y.append(0)
            else:
                pred_y.append(1)
