...
class KMeans:
    ...
    def dist_pt_to_centroids(self, pt, centroids):
        ...
        only a small fraction of the time for your code to stop running)
        '''
        dist = np.sqrt(np.sum(((centroids - pt)**2), axis = 1))
        return dist
    ...
    def initialize(self, k):
        ...
        '''
        initial = np.ones((k, self.num_features))
        for i in range(k):
            initial[i] = self.data[np.random.randint(0,self.num_samps)]
        return initial
    ...
    def cluster(self, k=2, tol=1e-2, max_iter=1000, verbose=False, p=2):
        ...
        labelList = []
        diffList = []
        centroidList.append(self.initialize(k).tolist())
        while count < max_iter:
            labels = self.update_labels(centroidList[count])
            labelList.append(labels)
            newCent, diff = self.update_centroids(k, labelList[count], centroidList[count])
            diffList.append(diff.tolist())
            centroidList.append(newCent.tolist())
            count += 1
            if np.absolute(np.array(diffList[-1]).all()) < tol:
                break
        self.data_centroid_labels = np.array(labelList[-1])
        self.centroids = np.array(centroidList[-1])
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False, p=2):
        ...
        for i in range(0, n_iter):
            inertia, its = self.cluster(k = k, verbose = verbose)
            inertiaList.append(inertia)
            centroidList.append(self.centroids)
            labelsList.append(self.data_centroid_labels)
        index = inertiaList.index(np.min(inertiaList))
        self.inertia = np.array(inertiaList[index])
        self.centroids = np.array(centroidList[index])
        self.data_centroid_labels = np.array(labelsList[index])
        if verbose:
            print('debug')
    ...
    def update_labels(self, centroids):
        ...
        for i in range(len(self.data)):
            distance = self.dist_pt_to_centroids(self.data[i], centroids)
            labels.append(np.where(distance == np.min(distance))[0][0])
    ...
    def update_centroids(self, k, data_centroid_labels, prev_centroids):
        ...
        for j in range(0, k):
            clusterPoints.clear()
            for i in range(0, len(data_centroid_labels)):
                if data_centroid_labels[i] == j:
                    clusterPoints.append(self.data[i].tolist())
            if np.sum(data_centroid_labels == j) > 0:
                mean = np.mean(clusterPoints, axis = 0)
                new_centroids[j] = mean
            else:
                new_centroids[j] = self.data[np.random.randint(0,len(data_centroid_labels))]
    ...
    def compute_inertia(self):
        ...
        centroidDistances = []
        distance = 0
        for i in range(0,np.max(self.data_centroid_labels)+1):
            for j in range(0, len(self.data_centroid_labels)):
                if self.data_centroid_labels[j] == i:
                    distance = self.data[j] - self.centroids[i]
                distance = (np.sqrt(np.sum(distance**2))**2) / len(self.data_centroid_labels)
                centroidDistances.append(distance)
        inertia = np.sum(centroidDistances)
    ...
    def plot_clusters(self):
        ...
        x = []
        y = []
        indicies = np.ones((1,1))
        for i in range(0, self.k):
            x.clear()
            y.clear()
            indicies = np.where(self.data_centroid_labels == i)
            for ind in indicies[0]:
                x.append(self.data[ind][0])
                y.append(self.data[ind][1])
            plt.scatter(x, y,  edgecolor = 'k', c = colors[i], label=str(i))
            plt.scatter(self.centroids[i][0], self.centroids[i][1], c = 'k', s = 100)
        plt.title('Clusters')
        plt.xlabel('X')
        plt.ylabel('Y')
    ...
    def elbow_plot(self, max_k, n_iter = 1):
        ...
        for i in range(1, max_k+1):
            ...
            inertiaList.append(self.inertia)
        plt.plot(np.arange(1,max_k+1), inertiaList, '.-')
    ...
    def replace_color_with_centroid(self):
        ...
        None
        '''
        label = self.update_labels(self.centroids)
        for j in range(0, self.k):
            for i in range(0, len(label)):
                if label[i] == j:
                    self.data[i] = self.centroids[j]
            if np.sum(label == j) == 0:
                print('none')
